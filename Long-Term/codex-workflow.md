...
- The canonical user-wide CI log triage skill is `/Users/jnyjc2/.codex/skills/ci-log-triage`. Use it for GitLab CI, Terratest, Terragrunt, Terraform, apply, destroy, or verbose pipeline failure investigations before relying on older repo-local copies.
- The canonical user-wide Jira MCP formatting skill is `/Users/jnyjc2/.codex/skills/jira-mcp-formatting`, with both `SKILL.md` and `agents/openai.yaml`. If autocomplete or implicit invocation does not pick up its metadata, restart Codex or open a new thread before editing the skill.
