...
- Keep `external_locations.yml` terse. Terragrunt should derive one storage credential per location from each row's `aws_iam_role_arn`, expand a `product_key` assignment to all active env-cycle service principals, and avoid a separate storage-credential map until real sharing needs appear.
