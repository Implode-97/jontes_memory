...
- Configure workspace-level Databricks providers with the workspace `host`; do not add `workspace_id` to those provider blocks unless current official provider documentation proves it is supported and needed. Keep module inputs such as `workspace_ids` and the implicit-binding anchor `provider_workspace_id` separate from provider configuration.
...
## Source-Owned External Locations

- For `sandbox_catalog` V1 `dbx_external_locations`, keep the feature standalone in regional `dbcore`: source teams provide S3 URLs and external AWS IAM role ARNs, while the platform creates and owns the Unity Catalog storage credentials, external locations, and grants. Source-side S3, IAM, bucket policies, and optional queues stay outside the platform mutation boundary.
- V1 should hard-code `read_only = true` and `skip_validation = true` in wrapper and guard logic, not expose them as YAML knobs. Terraform applies should not fail because source-owned buckets, roles, policies, or queues are not ready; source teams own runtime S3/IAM/eventing failures unless the feature contract changes.
- Default external locations to all-workspaces/open scope. Do not grant teams or products storage-credential privileges. External-location creator privileges should stay narrow and object-creation oriented for V1: `CREATE_EXTERNAL_TABLE` and `CREATE_EXTERNAL_VOLUME`; no default `READ_FILES`, `WRITE_FILES`, `MANAGE`, `ALL_PRIVILEGES`, `CREATE_MANAGED_STORAGE`, or `EXTERNAL_USE_LOCATION`. Avoiding `READ_FILES` preserves the intended Unity Catalog volume activation path instead of granting broad direct file reads.
- Keep `external_locations.yml` terse. Terragrunt should derive one storage credential per location from each row's `aws_iam_role_arn` and grant creator privileges only to team contributor groups derived from `assignments.teams`. `assignments.data_products` is unsupported for this feature for now and should fail clearly at the guard boundary. If product service principals later need external-location creator grants, build a separate bridge unit depending on both external locations and data product catalogs.
- File events must be explicitly disabled by default as `file_events = null`. Model opt-in as a complete object: `mode = "managed_sqs"` for the Databricks-managed automatic queue path with no queue URL, or `mode = "provided_sqs"` only when a source-owned SQS queue URL is supplied. Do not expose a `managed_sqs_queue_url` override without a real caller. Surface both Databricks `external_id` and `unity_catalog_iam_arn` so source teams can update trust policies.
- Canonicalize an external-location S3 URL once from caller input and reuse that desired value for the external location and any downstream catalog `storage_root`. Do not feed provider-refreshed URL outputs back into desired configuration: the provider can normalize `s3://bucket` to `s3://bucket/`, producing double slashes and forced catalog replacement. Cover this with a trailing-slash provider-output mock.
- The implementation plan at `/Users/jnyjc2/code/vibe_code/sandbox_catalog/external-location-feature-plan.md` passed the June 2026 scoring loop after cleanup. Before implementation, remove embedded review-score metadata, keep wrapper outputs narrow to consumed handoff/support fields and contract assertions, explain `provider_workspace_id` as the workspace-provider anchor even in all-workspaces mode, keep optional `file_events` examples separate, and make E2E a contract smoke rather than an S3/IAM readability proof.

## Unity Catalog Volumes And FUSE

- For Unity Catalog volumes, FUSE, Spark, and auth explanations, keep conceptual runtime communication paths separate from Terraform, Terragrunt, sandbox catalog, wrapper-module, or current implementation context unless explicitly requested. Center diagrams on driver-local native OS reads, FUSE/POSIX `/Volumes` access, Spark datasource reads through driver and executors, direct cloud URI access, Unity Catalog auth/policy checks, and cloud storage credential boundaries. Label inferred Databricks internals as conceptual abstractions and keep caveats about executor-native paths, RDDs, UDFs, access modes, and runtime versions visually secondary.
- For data under a Unity Catalog external volume, cloud URI access such as `s3://...` is governed by the external volume's privileges, not by `READ_FILES` or `WRITE_FILES` on the parent external location. Direct external-location privileges apply to path access outside a table or volume object. Normal volume reads require `USE CATALOG`, `USE SCHEMA`, and `READ VOLUME`; writes also require `WRITE VOLUME`.
- When explaining Spark access to Unity Catalog volumes, separate the authentication path from the performance path. Native `spark.read` over `/Volumes` or `dbfs:/Volumes` is a Spark datasource scan where the driver plans listing, filters, file partitions, and executor reads. Opening each file inside a Spark UDF is custom task I/O, so splitting, grouping, pruning, metrics, and failure behavior can differ even when both paths touch the same backing storage.
...
- For CAV-134270 data product catalogs, keep the feature separate from sandbox catalogs. Each `product_key + env_cycle` should get its own service principal, federation policies, bucket, external location, and catalog; the metastore owner group owns the catalog while the product service principal receives `ALL_PRIVILEGES` and `MANAGE`.
...
## SQL Warehouses And Cost Tags

- For Databricks SQL warehouse, serverless budget, cluster policy, and other compute-tagging modules, required platform attribution tags are not enough. Validate provider-compatible custom tag keys and values early: no spaces or `/`, only the supported AWS tag character set, and reserved keys such as `.`, `..`, and `_index` should be rejected before provider/API apply. Prefer compact invalid-input tests for representative invalid characters and reserved keys.
...
- Do not treat an intermediate Databricks service credential or instance-profile role with negative IAM permissions as a durable guardrail once user code can assume another AWS role. Role chaining does not make source-role S3 denies carry into the target role session. Put enforceable controls on the final role session, target role policy or permissions boundary, S3 bucket/resource policy, AWS Organizations SCP/RCP guardrails, or Unity Catalog primitives that avoid exposing broad raw cloud credentials. Broad `sts:AssumeRole` from Databricks credentials is a governance escape hatch.
...
- In `everything_app`, prefer `st.dataframe` row selection with stable widget keys and session-state-backed text inputs for interactive Unity Catalog browsing. Selecting catalogs/schemas/tables/volumes should populate dependent inputs and clear stale lower-level fields; build volume paths as `/Volumes/{catalog}/{schema}/{volume}`.
- In the same Streamlit PoC, keep action buttons in compact columns but render query results, errors, and dataframes below the button row in a shared full-width area. Rendering `_show_result()` inside a clicked button column makes wide Unity Catalog table previews and table lists unusable; use `st.dataframe(..., width="stretch", height=600)` for result-heavy views.
...
- In `/Users/jnyjc2/code/innovation-week/refactor_aula`, treat root `app.py` plus lowercase `aula_app/` as the active greenfield app. Legacy top-level `Pages/`, `Utilities/`, `Resources/`, `Queries/`, `dst_data/`, `app_runner.py`, and `sync_files*.py` were removed; query files needed by the app live under `aula_app/queries/`.
- Keep `refactor_aula` pass-through-only unless the user explicitly changes the security model. Forwarded `x-forwarded-access-token` handling belongs in `aula_app/auth.py`; SQL calls should use the Databricks SQL connector with `access_token=user.access_token`; Files API access should use an explicit user bearer token; and the app should not reintroduce default `WorkspaceClient()`, `DATABRICKS_TOKEN`, token-bearing Streamlit caches, raw annotation SQL literals, or app service-principal SQL/UC/volume resources.
- `refactor_aula` annotations are keyed by `sequence_id`, `camera_id`, and `attribute_name`, with typed `AnnotationRow` validation and `ImageKey` identities that include both `sequence_id` and `camera_id`. Do not restore sequence-only image maps or page-level annotation SQL from legacy modules.
- `refactor_aula` should preserve the `everything_app` warehouse selector config: default warehouse `ATS Warehouse` with ID `30c57a3c1e729b60` plus the selectable warehouse JSON in both `databricks.yml` and `app.yaml`. App serverless budget policy config uses required bundle variable `firecrackers_budget_policy_id`; do not give it a fake default.
- For `refactor_aula` DAB work, keep `bundle.engine: direct` because the Terraform bundle engine failed on an expired OpenPGP checksum-signature key while downloading Terraform. Use only accepted app `user_api_scopes` such as `sql` and `files.files`; the Apps create API rejected `iam.current-user:read` even though existing metadata may show it as effective.
- In Streamlit apps where pages import callables all named `render`, set explicit `url_path` values such as `start`, `owl-bbox-labels`, and `clip-image-labels`. Callable-name inference can otherwise produce duplicate `render` routes.
