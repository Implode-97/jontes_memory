...
- Default external locations to all-workspaces/open scope. Do not grant teams or products storage-credential privileges. External-location creator privileges should stay narrow and explicit: `READ_FILES`, `CREATE_EXTERNAL_TABLE`, and `CREATE_EXTERNAL_VOLUME`; no `WRITE_FILES`, `MANAGE`, `ALL_PRIVILEGES`, `CREATE_MANAGED_STORAGE`, or `EXTERNAL_USE_LOCATION` by default.
...
- File events must be explicitly disabled by default and modeled only as a complete opt-in object with provided queue/handoff data. Surface both Databricks `external_id` and `unity_catalog_iam_arn` so source teams can update trust policies.
